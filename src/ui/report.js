...
import { escapeHtml, e, escapeDeep, safeHref, dataCopyAttr } from '../utils/sanitize.js';
...
export function renderReportContent() {
  ...
  const bundles = generateBundleStructure(report);
  ...
  const htmlBlocks = generateHTMLConversionBlocks(report);
  ...
  container.innerHTML = `
    ...
    <section id="section-bundle-builder" class="report-section hidden">
      ...
      <p class="report-section-desc">Diseño de paquetes de producto (1x, 2x, 3x) para disparar el Ticket Medio (AOV). Precios y AOV salen del motor de bundles según el retail del informe (heurística offline, no predicción de ventas).</p>
      ...
      <div style="background: rgba(139, 92, 246, 0.08); border: 1px solid rgba(139, 92, 246, 0.3); border-radius: 8px; padding: 1.25rem; margin-bottom: 1.5rem; display: flex; align-items: center; justify-content: space-between; flex-wrap: wrap; gap: 1rem;">
        <div>
          ...
          <h3 style="font-size: 1.4rem; color: var(--accent-emerald); margin: 0.2rem 0 0 0;">+${bundles.aovBoostPercent}% AOV estimado</h3>
        </div>
        <div style="font-size: 0.85rem; color: var(--text-secondary); max-width: 400px;">
          Con mix heurístico 35% / 50% / 15% entre packs 1x / 2x / 3x, el ticket medio sube de <strong>$${bundles.pack1.totalPrice.toFixed(2)}</strong> a <strong>$${bundles.weightedAov.toFixed(2)}</strong>.
        </div>
      </div>
    ...
    <section id="section-html-blocks" class="report-section hidden">
      ...
      <div class="prompt-card" style="margin-bottom: 1.5rem;">
        <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:0.75rem;">
          ...
          <button class="btn btn-secondary btn-sm btn-copy-clipboard" data-copy="${dataCopyAttr(htmlBlocks.comparisonTableHtml)}">
      ...
      <div class="prompt-card" style="margin-bottom: 1.5rem;">
        <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:0.75rem;">
          ...
          <button class="btn btn-secondary btn-sm btn-copy-clipboard" data-copy="${dataCopyAttr(htmlBlocks.benefitsGridHtml)}">
            <i data-lucide="copy"></i> Copiar Código HTML
          </button>
        </div>
      ...
      <div class="prompt-card">
        <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:0.75rem;">
          ...
          <button class="btn btn-secondary btn-sm btn-copy-clipboard" data-copy="${dataCopyAttr(htmlBlocks.faqAccordionHtml)}">
            <i data-lucide="copy"></i> Copiar Código HTML
          </button>
        </div>
...
export function renderPrintableReport() {
  ...
  printContainer.innerHTML = `
    ...
    <div class="print-page">
      ...
      <ul>
        ${report.competitorAnalysis.competitorsGanchos.map(g => `<li>${e(g)}</li>`).join('')}
      </ul>
      <p><strong>Nuestros Ganchos de Desvío:</strong></p>
      <ul>
        ${report.competitorAnalysis.ourGanchos.map(g => `<li>${e(g)}</li>`).join('')}
      </ul>
